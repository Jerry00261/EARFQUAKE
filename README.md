...
│   │   ├── ml/              # Machine learning models & ROM
│   │   └── schemas/         # Pydantic response models
│   ├── data/                # Seismic datasets (CSV, NumPy)
│   ├── scripts/             # DB seeding & heatmap generation
...
| `GET` | `/api/v1/earthquakes` | List earthquakes with optional filters (year, magnitude, site class, place) |
| `GET` | `/api/v1/earthquakes/summary` | Aggregate stats for a given year |
| `GET` | `/api/v1/earthquakes/{id}` | Single earthquake detail |
| `GET` | `/api/v1/datasets` | Dataset catalog with document counts |
| `GET` | `/api/v1/datasets/source-locations` | List ROM source location parameters |
| `GET` | `/api/v1/datasets/receivers` | List receiver station summaries |
| `GET` | `/api/v1/datasets/receivers/{index}` | Full receiver waveform data |
| `GET` | `/api/v1/ml/predict` | ML risk prediction for a lat/lon coordinate |
| `GET` | `/api/v1/ml/heatmap` | Precomputed seismic hazard grid |
| `GET` | `/api/v1/ml/seismogram` | Synthetic waveform via ROM for a source location |
| `GET` | `/api/v1/ml/waveform` | Screen-shake pixel offsets for animation |
